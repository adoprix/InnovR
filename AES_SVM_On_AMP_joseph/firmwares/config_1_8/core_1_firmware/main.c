...
int main(void)
{
    ...
    uint32_t t_aes_begin, t_write, t_aes_end, counter, t_send_begin,  t_send_end, t_receive_begin, t_receive_end;
    double lat_aes_ms = 0;
    double lat_write = 0;
    double t_send_ms = 0;
    ...
    while(1) {
        ...
        if(cmd_rx != AMP_NULL)
        {
            ...
            if(sel_op  == 1){
                ...
                force_ccr_write_data(&priv_data, sizeof(priv_data), CCR_N-1);
                t_write = amp_millis();

                time_spent_ms = (t_aes_begin - t_aes_end)/(CONFIG_CLOCK_FREQUENCY/1000.0);
                lat_aes_ms += time_spent_ms;

                time_spent_ms = (t_aes_end - t_write)/(CONFIG_CLOCK_FREQUENCY/1000.0);
                lat_write += time_spent_ms;
