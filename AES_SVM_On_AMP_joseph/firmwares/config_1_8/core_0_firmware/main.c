...
static void console_service(void)
{
    ...
    if(strcmp(token, "help") == 0)
        ...
        uint32_t t_svm_begin, t_svm_end,  t_send_begin, t_send_end, t_read_end;
        float time_spent_ms;
        uint8_t class;
        ...
        for (int i = 0; i < MEASURE_STEPS; i++)
        {
            ...
            force_ccr_read_data(&f_img, sizeof(f_img), CCR_N -1);
            t_read_end = amp_millis();
            ...
            time_spent_ms = (time_begin - t_read_end)/(CONFIG_CLOCK_FREQUENCY/1000.0);
            lat_read += time_spent_ms;
        ...
        time_spent_ms = lat_read/MEASURE_STEPS;
        f_left = (int)time_spent_ms;
        f_right = ((float)(time_spent_ms - f_left)*1000.0);
        printf("Total Read time  is : %d.%d ms\n", f_left, f_right);
